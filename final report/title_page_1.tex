...
No algorítmo de arrefecimento simulado a maior dificuldade foi encontrar uma representação que permitisse a geração de novos estados e alocação de elementos às tasks de maneira eficiente e rápida.
...
\begin{lstlisting}[frame=single, language=java,numbers=left,numberfirstline=true] 
public float eval(Solution solution){
	float bigger = 0;
	for(Task task : solution.getOrderedTasks()){
		float completionTime = solution.getTaskCompletionTime(task);
...
\justify\normalsize
Para tornar mais fácil a visualização dos resultados foi usada uma API de visualização de grafos chamada GraphStream. A linguagem usada foi Java.
...
Do lado direito da figura pode-se observar o grafo de precedências de tarefas. Do lado esquerdo apresenta-se o gráfico que representa a evolução do valores gerados pelos novos estados à medida que a temperatura se aproxima de 0. A figura \ref{sa_graph1} mostra o gráfico com mais detalhe.
...
Foi gerado um problema de forma automática com 25 tarefas, 8 skills e 10 elementos, com atribuição de skills a tarefas e elementos e criação de precedências entre tarefas.
Este problema foi utilizado para estudar várias configurações para os algorítmos e para os comparar com o problema de maior dimensão que o da experiência  1.
...
\begin{mycapequ}[!ht]
  \begin{equation}
   f(a,b, \ldots)  = { t \over p_a  + p_b  +\ldots } + g(a,b,\ldots)
  \end{equation}
  \caption{ i - tempo actual\\
  		g(a,b,\ldots) - tempo a esperar até que todos os elementos estivessem disponíveis
  		}
\end{mycapequ}

Fazendo uma análise da função poderíamos utilizar esta heurística para optimizar ambos os algorítmos e chegar assim a soluções que se aproximassem mais do óptimo global

\subsection{Outros Melhoramentos}
Outros melhoramentos a serem implementadas passariam por realizar mais testes por forma a perceber quais as condições que levariam a uma mehor otimização.

\section{Divisão de tarefas}
\justify\normalsize

O grupo considera que o trabalho foi desenvolvido de uma maneira saudável e todos os elementos do grupo trabalharam de forma igual (33.3\% para cada elemento). %% Não concordo, o SA tá op mereço mais
