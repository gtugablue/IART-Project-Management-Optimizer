...
%Igual ao relatório passado
\paragraph{Geração do Próximo Estado}
%igual ao relatório passado
\paragraph{Atribuição do Tempo inicial e dos Elementos às Tarefas }
%ver função no ScheduleGenerationScheme chamada assignElements(acho que é este o nome)
\paragraph{Calculo de duração de uma tarefa}
%Tempo da tarefa = 1/(1/x+1/y) em que x e y é o tempo que o funcionário x e o funcionário y demoram a executar a tarefa.
%x = duração da tarefa / performance de x 
...
T \textsubscript{n} = 	\alpha  * T\textsubscript{n-1}
\end{equation}
